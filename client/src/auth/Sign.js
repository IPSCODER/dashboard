	...
	const submitHandler = async (e) =>{
		...
		if (!sign) {
			if (!emailRegex.test(email)) {
				...
				await axios.post('http://localhost:5000/user/signin',{email,password}).then((resp)=>{
			...
			if (!usernameRegex.test(username)) {
				...
				await axios.post('http://localhost:5000/user/signup',{username,email,password}).then((resp)=>{
				if (resp.data.result == true) {
					navigate("/")
					dispatch(loadGetData())
					dispatch(loadUser(email))
